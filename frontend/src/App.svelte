...
<div class="shell">
  ...
  <div class="rail">
    ...
    <button class="rail-btn" title={theme === 'dark' ? 'Switch to light mode' : 'Switch to dark mode'} onclick={toggleTheme}>
      ...
      {:else}
        <!-- Moon icon -->
        <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>
      {/if}
    </button>
    ...
    <button class="rail-btn" title="Help (coming soon)" disabled>
      <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75"><circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
    </button>
  </div>
...
<style>
  ...
  .channel-hash { color: var(--channel-hash); font-size: 1.1rem; flex-shrink: 0; }
  .channel-name {
    font-size: 0.9375rem;
    font-weight: 600;
  ...
  .channel-name-placeholder { font-size: 0.9375rem; color: var(--text-muted); }
  .channel-desc {
    font-size: 0.8125rem;
